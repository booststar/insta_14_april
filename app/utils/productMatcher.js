...
export function detectProductMatches(posts = [], products = [], existingTaggedProducts = {}) {
  ...
  posts.forEach((post) => {
    ...
    processedProducts.forEach((prod) => {
      ...
      // 1. Exact phrase or compact title in caption
      ...
      // 2. Compact Handle match (e.g. #blowoutbabethermalbrush or in text)
      ...
      // 3. Core Keywords Match
        ...
        if (prod.coreKeywords.length === 1 && matchedKeywords.length === 1 && prod.coreKeywords[0].length >= 4) {
          confidence = 82;
          ...
          confidence = matchRatio === 1 ? 92 : 86;
          matchReason = `Matched key terms: ${matchedKeywords.join(", ")}`;
          matchedTerm = matchedKeywords.join(" ");
        } else if (matchedKeywords.length >= 1 && (prod.coreKeywords.length <= 2 || matchedKeywords.some(k => k.length >= 6))) {
          confidence = 72;
          const keyTerm = matchedKeywords.find(k => k.length >= 6) || matchedKeywords[0];
          matchReason = `Matched keyword "${keyTerm}" in caption`;
          matchedTerm = keyTerm;
        }
      }

      // 4. Tag / Category Match
      if (confidence < 70 && prod.tags.length > 0) {
